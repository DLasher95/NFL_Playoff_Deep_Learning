...
public class MatrixFromToolKit {
	// Data
	...
	public MatrixFromToolKit() {}

	// Copies the specified portion of that matrix into this matrix
	public MatrixFromToolKit(MatrixFromToolKit that, int rowStart, int colStart, int rowCount, int colCount) {
	...
	public void add(MatrixFromToolKit that, int rowStart, int colStart, int rowCount) throws Exception {
	...
	void shuffle(Random rand, MatrixFromToolKit buddy) {
		for (int n = rows(); n > 0; n--) {
			int i = rand.nextInt(n);
			double[] tmp = row(n - 1);
			m_data.set(n - 1, row(i));
